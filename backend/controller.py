...
class ChatManager:
    ...
    def load_chat_list(self, user):
        ...
        print(user)
        if user:
            return function.load_chat_list(user)
        return [] # Or handle error differently if preferred
    def load_message_user(self, user, chat):
        user = function.load_user(user)
        if user:
            return function.load_messages_user(user, chat)
        else:  {"status": "error", "message": "Failed to load message.", "output": False, "running": False}
    
    def load_message_group(self, group_name, user):
        user = function.load_user(user)
        if user:
            return function.load_messages_group(group_name, user)
        else:  {"status": "error", "message": "Failed to load message.", "output": False, "running": False}
